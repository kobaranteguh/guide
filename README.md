...
- 📤 SENDING TO A CUSTOMER WITH NO PHONE NUMBER: Every Bridge send endpoint accepts EITHER 'to' (phone number) OR 'recipient' (BSUID). When a customer has adopted a WhatsApp username, Meta stops sending their phone number and we only have a BSUID - sending 'to' with an empty or missing value fails, so use 'recipient' instead. Store the canonical id per contact and choose at send time: if we hold a real phone number use 'to', otherwise use 'recipient'. When both are supplied 'to' wins (Meta's rule). IMPORTANT: a send to 'recipient' returns contacts[0].user_id and NO wa_id, so any code reading wa_id from a send response must read user_id for BSUID sends. Also note Meta REJECTS BSUID recipients for one-tap, zero-tap and copy-code AUTHENTICATION templates with error 131062 - this is permanent, so any OTP or verification flow must always collect and use a real phone number.
...
STEP 3 - IMPLEMENT, in this order, only after I approve:
  ...
  (f) SEND TO BSUID - unblocks replies to username customers.
      Check whether our send calls only ever pass 'to'. Bridge 2.4.0 accepts 'recipient' (a BSUID) as an alternative on every send endpoint, including broadcasts. Without it we cannot reply at all to a customer whose phone number Meta no longer sends.
      Choose at send time: use 'to' when we hold a real phone number, 'recipient' when we only have a BSUID. When both are passed 'to' wins, so adding 'recipient' cannot change existing behaviour.
      Two traps: (1) a send to 'recipient' returns contacts[0].user_id and NO wa_id - fix any code that reads wa_id from a send response to record the recipient; (2) Meta REJECTS BSUID recipients for one-tap, zero-tap and copy-code AUTHENTICATION templates with error 131062, permanently - so OTP and verification flows must keep collecting a real phone number.
